        ...
        <div class="mission-layout h-100 d-flex flex-column flex-lg-row">
            ...
            <div class="mission-sidebar d-flex flex-column bg-white z-3 shadow-lg">
                <div class="sidebar-header p-4 pb-3 border-bottom position-relative overflow-hidden">
                    <div class="bg-shape bg-primary rounded-circle position-absolute opacity-10"
                        style="width: 150px; height: 150px; top: -50px; right: -50px;"></div>

                    <div class="d-flex align-items-center mb-3 position-relative z-1">
                        <div class="icon-box bg-primary bg-opacity-10 text-white me-3 d-flex align-items-center justify-content-center rounded-3"
                            style="width: 48px; height: 48px;">
                            <i class="bi bi-radar fs-4"></i>
                        </div>
                        <div>
                            <h5 class="fw-bolder mb-1 text-dark">Radar Cứu Hộ</h5>
                            <span class="text-muted small fw-medium">Có {{ radarAssignments.length }} nhiệm vụ chờ tiếp
                                nhận</span>
                        </div>
                    </div>

                    <!-- Active request warning banner -->
                    <div v-if="hasActiveRequest"
                        class="alert alert-danger py-2 px-3 mb-0 small fw-bold d-flex align-items-center gap-2 position-relative z-1">
                    ...
                    <div class="d-flex gap-2 position-relative z-1 overflow-auto pb-1 hide-scrollbar">
                        <button class="btn btn-sm btn-dark rounded-pill px-3 fw-bold shadow-sm"
                            @click="activeTab = 'all'">Tất cả</button>
                        <button class="btn btn-sm bg-danger bg-opacity-10 text-white border-0 rounded-pill px-3 fw-bold"
                            @click="activeTab = 'critical'">Khẩn cấp</button>
                    </div>
                </div>

                <div
                    class="mission-list flex-grow-1 overflow-auto p-3 d-flex flex-column gap-3 bg-secondary bg-opacity-10">
                    <!-- Loading -->
                    ...
                    <div v-else v-for="item in displayAssignments" :key="item.id_phan_cong"
                        class="mission-card card border-0 shadow-sm rounded-4 transition-all"
                        :class="getCardClass(item)"
                        :style="selectedMission && selectedMission.id_phan_cong === item.id_phan_cong ? 'background: linear-gradient(135deg, rgba(37, 99, 235, 0.15), rgba(37, 99, 235, 0.05)); border: 2px solid #2563eb !important;' : ''"
                        @click="selectMission(item)">
                        <div class="card-body p-4">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <span
                                    :class="['badge rounded-pill text-white px-3 py-2 fw-bold border', getSeverityBadgeClass(item)]">
                                    <i class="bi bi-exclamation-triangle-fill me-1"
                                        v-if="getSeverityLabel(item) === 'KHẨN CẤP'"></i>
                                    {{ getSeverityLabel(item) }}
                                </span>
                                <div class="text-end">
                                    <div class="fw-bold text-dark fs-5">{{ item.yeu_cau && item.yeu_cau.muc_do_khan_cap
                                        ? item.yeu_cau.muc_do_khan_cap : '-' }}</div>
                                    <div class="text-muted small fw-medium"><i
                                            class="bi bi-clock me-1 text-primary"></i>{{ item.created_at ?
                                        formatTime(item.created_at) : '-' }}</div>
                                </div>
                            </div>
                            <h6 class="fw-bold fs-6 mb-2 text-dark">
                                {{ getIncidentTypeName(item) }}
                            </h6>
                            <div class="text-muted small mb-2"><i class="bi bi-hash me-1"></i>ID: {{ item.yeu_cau &&
                                item.yeu_cau.id_yeu_cau ? item.yeu_cau.id_yeu_cau : '-' }}</div>
                            ...
                            <button v-if="item.trang_thai_nhiem_vu === 'MOI'"
                                class="btn btn-primary w-100 fw-bold btn-lg rounded-3 py-2 btn-accept shadow-sm"
                                :class="{ 'disabled': hasActiveRequest }" :disabled="hasActiveRequest"
                                @click.stop="acceptMission(item)">
                                <i class="bi bi-check-circle me-2"></i>TIẾP NHẬN NGAY
                            </button>
                            <button v-else class="btn btn-outline-secondary w-100 fw-bold btn-lg rounded-3 py-2"
            ...
            <div class="map-area flex-grow-1 position-relative bg-light">
                ...
                <div class="mission-popup-card position-absolute m-3 m-lg-4 z-3 rounded-4" style="max-width: 360px;"
                    v-if="selectedMission">
                    <div class="card border-0 shadow-lg bg-white rounded-4 overflow-hidden">
                        <div
                            class="card-header bg-transparent border-bottom-0 pb-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                            ...
                            <button class="btn btn-sm btn-icon btn-light rounded-circle text-muted"
                                @click="selectedMission = null"><i class="bi bi-x-lg"></i></button>
                        </div>
                        ...
                        <div class="card-body px-4 pb-4 pt-3">
                            ...
                            <div class="victim-info bg-light rounded-3 p-3 mb-3 border border-light shadow-sm"
                                v-if="selectedMission.yeu_cau">
                                <div class="d-flex align-items-center mb-2">
                                    <div class="avatar-circle bg-white text-primary shadow-sm me-3 fw-bold fs-5 border d-flex align-items-center justify-content-center"
                                        style="width: 42px; height: 42px; border-radius: 50%;">
                                        {{ (getReporterName(selectedMission) || 'N').charAt(0).toUpperCase() }}
                                    </div>
                                    <div>
                                        <div class="text-muted fw-bold" style="font-size: 10px; letter-spacing: 0.5px;">
                                            NGƯỜI GẶP NẠN</div>
                                        <div class="fw-bold text-dark">{{ getReporterName(selectedMission) }}</div>
                                    </div>
                                </div>
                                <div class="small text-secondary mb-2"
                                    v-if="selectedMission.yeu_cau.so_dien_thoai_nguoi_dung || getReporterPhone(selectedMission)">
                                    <i class="bi bi-telephone me-1"></i>
                                    {{ selectedMission.yeu_cau.so_dien_thoai_nguoi_dung ||
                                    getReporterPhone(selectedMission) }}
                                </div>
                                ...
                                <div class="d-flex gap-2 mt-3">
                                    ...
                                    <a v-if="getRequestAddress(selectedMission)"
                                        ...
                                        target="_blank" class="btn btn-danger rounded-3 btn-sm fw-bold flex-grow-1">
                            ...
                            <div class="mb-3" v-if="selectedMission.yeu_cau && selectedMission.yeu_cau.mo_ta">
                                <div class="text-muted fw-bold mb-1" style="font-size: 10px; letter-spacing: 0.5px;">MÔ
                                    TẢ</div>
                            ...
                            <button v-else @click="goToMission(selectedMission)"
                                class="btn btn-danger w-100 py-3 fw-bold rounded-3 shadow-danger">
                ...
                <!-- Map Controls -->
                <div
                    class="map-controls position-absolute bottom-0 end-0 m-4 d-flex flex-column gap-2 z-3 d-none d-md-flex">
                    <button
                        class="btn btn-white shadow-sm bg-white p-2 border-0 rounded-3 text-dark btn-map-control hover-bg-gray"
                        @click="zoomIn"><i class="bi bi-plus-lg"></i></button>
                    <button
                        class="btn btn-white shadow-sm bg-white p-2 border-0 rounded-3 text-dark btn-map-control hover-bg-gray"
                        @click="zoomOut"><i class="bi bi-dash-lg"></i></button>
                    <button
                        class="btn btn-white shadow-sm bg-white p-2 border-0 rounded-3 text-primary mt-2 btn-map-control hover-bg-gray"
                        @click="locateMe"><i class="bi bi-crosshair"></i></button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed: {
        // radarAssignments: only show pending assignments for the current team
        // that are NOT already being processed (DANG_XU_LY/DA_DEN_HIEN_TRUONG)
        // by any team. This prevents showing the same request from multiple teams.
        radarAssignments() {
            const urgencyOrder = { 'CRITICAL': 4, 'HIGH': 3, 'MEDIUM': 2, 'LOW': 1 };
            const teamId = this.teamId;

            // Collect all yeu_cau IDs that are currently being processed (DANG_XU_LY/DA_DEN_HIEN_TRUONG)
            ...
            const filtered = this.assignments.filter(a => {
                ...
                // Filter: status must be DA_PHAN_CONG or MOI (awaiting acceptance)
                const st = (a.trang_thai_nhiem_vu || '').toUpperCase().replace(/\s+/g, '_');
                if (st !== 'DA_PHAN_CONG' && st !== 'MOI') return false;
                // Filter: hide if the yeu_cau is already being processed by any team
                const ycId = a.yeu_cau?.id_yeu_cau;
                if (ycId && processingYeuCauIds.has(ycId)) return false;
                return true;
            });

            // Deduplicate: keep only the latest assignment per id_yeu_cau
            ...
            console.log('[TrangChu] radarAssignments input:', filtered.length, '| deduped:', deduped.length);

            // Sort by emergency level first (CRITICAL > HIGH > MEDIUM > LOW),
    ...
    methods: {
                ...
                if (this.teamId) {
                    ...
                    if (res.data) {
                        ...
                        if (Array.isArray(rawItems)) {
                            all = rawItems;
                        } else if (Array.isArray(rawItems?.data)) {
                            all = rawItems.data;
                        }
                        console.log('[TrangChu] rawItems:', rawItems);
                    }
                } else {
                    const res = await rescuerAPI.getAssignments({ per_page: 100 });
                    console.log('[TrangChu] API response (no teamId):', res.data);
                    if (res.data) {
                        const rawItems = res.data.data ?? res.data;
                        if (Array.isArray(rawItems)) {
                            all = rawItems;
        ...
        updateMapMarkers() {
            ...
            this.radarAssignments.forEach(item => {
                if (item.yeu_cau && item.yeu_cau.vi_tri_lat && item.yeu_cau.vi_tri_lng) {
                    ...
                    const moTaPhanCong = item.mo_ta ? item.mo_ta : '';

                    marker.bindPopup(`
            <div style="min-width:220px">
              <h6 style="margin:0 0 8px;font-weight:bold;color:#1e293b">${typeName}</h6>
              <p style="margin:0 0 4px;font-size:12px;color:#2563eb;font-weight:600"><i class="bi bi-hash"></i> ID: ${item.yeu_cau && item.yeu_cau.id_yeu_cau ? item.yeu_cau.id_yeu_cau : '-'}</p>
              ...
              ${victims ? `<p style="margin:0 0 4px;font-size:12px;color:#dc2626"><i class="bi bi-people"></i> ${victims} người bị ảnh hưởng</p>` : ''}
              ${moTaPhanCong ? `<p style="margin:0;font-size:12px;color:#d97706;font-weight:600"><i class="bi bi-card-text"></i> Ghi chú: ${moTaPhanCong}</p>` : ''}
            </div>
          `);
        ...
        async selectMission(item) {
            ...
            if (item && item.yeu_cau && item.yeu_cau.vi_tri_lat && item.yeu_cau.vi_tri_lng) {
                if (this.teamLat && this.teamLng) {
                    await this.drawRoute(this.teamLat, this.teamLng, item.yeu_cau.vi_tri_lat, item.yeu_cau.vi_tri_lng);
                }
